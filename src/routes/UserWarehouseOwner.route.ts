...
import { UserWarehouseOwnerActions } from '@actions/User/UserWarehouseOwner.action';

const userWarehouseOwnerActions = new UserWarehouseOwnerActions();

router.get('/', async (req, res) => {
    try {
        let where = { active: true };
        const result = await userWarehouseOwnerActions.getAll(where);
        res.json(result);
    } catch (e) {
...
router.get('/getBy', async (req, res) => {
    try {
        ...
        const result = await userWarehouseOwnerActions.getById(id);
        res.json(result);
    } catch (e) {
        res.status(500).json({ message: UNEXPECTED_ERROR_MESSAGE });
        logger.error(TRY_ERROR_MESSAGE + 'get ' + TAG);
...
router.post('/create', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await userWarehouseOwnerActions.create(
                req.body.user,
                req.body.name,
                req.body.lastName,
...
router.put('/', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await userWarehouseOwnerActions.update(
                req.body.id,
                req.body.values);
            res.json(result);
        } else {
...
router.delete('/', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await userWarehouseOwnerActions.delete(req.body.id);
            res.json(result);
        } else {
            res.status(400).json({ message: BAD_REQUEST_MESSAGE });
        }
