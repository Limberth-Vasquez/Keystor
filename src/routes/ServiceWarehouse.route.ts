...
import { ServiceWarehouseActions } from '@actions/ServicesWarehouse/ServicesWarehouse.action';
...
router.get('/', async (req, res) => {
    try {
        ...
        const users = await serviceWarehouseActions.getAll(where);
        res.json(users);
    } catch (e) {
...
router.get('/getBy', async (req, res) => {
    try {
        ...
        const users = await serviceWarehouseActions.getById(id);
        res.json(users);
    } catch (e) {
        res.status(500).json({ message: UNEXPECTED_ERROR_MESSAGE });
        logger.error(TRY_ERROR_MESSAGE + 'get ' + TAG);
...
router.post('/create', async (req, res) => {
    try {
        if (req.body) {
            const requiredParams = ['description', 'typeServiceId', 'wareHouseId'];
...
router.put('/', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await serviceWarehouseActions.update(
                req.body.id,
                req.body.values);
            res.json(result);
        } else {
...
router.delete('/', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await serviceWarehouseActions.delete(req.body.id);
            res.json(result);
        } else {
            res.status(400).json({ message: BAD_REQUEST_MESSAGE });
        }
