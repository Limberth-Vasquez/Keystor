...
import { PublicityActions } from '@actions/Publicity/Publicity.action';

const publicityActions = new PublicityActions();

router.get('/', async (req, res) => {
    try {
        let where = { active: true };
        const result = await publicityActions.getAll(where);
        res.json(result);
    } catch (e) {
...
router.get('/getBy', async (req, res) => {
    try {
        ...
        const result = await publicityActions.getById(id);
        res.json(result);
    } catch (e) {
        res.status(500).json({ message: UNEXPECTED_ERROR_MESSAGE });
        logger.error(TRY_ERROR_MESSAGE + 'get ' + TAG);
...
router.post('/create', async (req, res) => {
    try {
        if (req.body) {
            const requiredParams = ['title', 'photo'];
            ...
            const result = await publicityActions.create(
                req.body.title,
                req.body.photo,
                req.body.link,
                req.body.photo2,
...
router.put('/', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await publicityActions.update(
                req.body.id,
                req.body.values);
            res.json(result);
        } else {
...
router.delete('/', async (req, res) => {
    try {
        if (req.body) {
            ...
            const result = await publicityActions.delete(req.body.id);
            res.json(result);
        } else {
            res.status(400).json({ message: BAD_REQUEST_MESSAGE });
        }
