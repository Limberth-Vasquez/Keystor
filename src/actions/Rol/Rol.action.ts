import { rolRepository } from '@shared/services/mongodb/repository/Rol.repository';
...
import { RolModel } from '@shared/services/mongodb/model/Rol.model';

const jwt = require('jsonwebtoken');
let ObjectId = require('mongodb').ObjectId;
let TAG = "Rol";

export class RolActions {
    public verifications: RolModel[];

    async create(name) {
        logger.info('action=create collection ' + TAG);
        try {
            await rolRepository.create(name);
    ...
    async  getAll(where = {}) {
        ...
        return await rolRepository.find(where).then(res => {
    ...
    async getById(_id: string) {
        ...
        let currentVerification = new RolModel();
        this.verifications = await rolRepository.find({ _id });
    ...
    async  update(_id: string, Rol: RolModel) {
        logger.info('action=update collection ' + TAG);
        _id = new ObjectId(_id);
        try {
            await rolRepository.update(Rol, { _id });
    ...
    async  delete(_id: string) {
        ...
        try {
            await rolRepository.delete(_id);
