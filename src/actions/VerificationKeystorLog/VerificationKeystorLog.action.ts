...
export class VerificationKeystorLogActions {
    ...
    async getById(_id: string) {
        ...
        let verificationKeystorLog = new VerificationKeystorLogModel();
        this.verifications = await verificationKeystorLogRepository.find({ _id });
        verificationKeystorLog = this.verifications.shift();
        if (verificationKeystorLog.active)
            return { valid: true, code: SUCCESS_CODE, data: verificationKeystorLog };
        else
